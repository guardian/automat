...
object FilterHelpers {
  ...
  private[this] def genMeta: Gen[Meta] =
    for {
      sectionId <- Gen.oneOf(
        "about",
        "animals-farmed",
    ...
    } yield Meta(sectionId)

  def sampleMeta(sectionId: Option[String] = None) = {
    val m = genMeta.sample.get
    m.copy(sectionId = sectionId.getOrElse(m.sectionId))
  }
}
